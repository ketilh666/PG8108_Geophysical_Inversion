...
def waxman_smits(So, Sg, phi, vcl, cec, zzz, **kwargs):
    """ Archie or Waxman-Smits model for resistivity
    ...
    ----------
        ...
        cec : array of floats. Cation exchange capacity. 
        zzz : array of floats. Depth below seabed
    All arrays must have same shape and size or scalar float
    ...
    **kwargs
    ...
    # Archie parameters (fixed, make kwargs later)
    a = 1          # Turtousity factor
    n = 2.0        # Saturation exponent
    m = 1.8        # Porosity exponent
    r_water = 0.06 # Water resistivity, from LFP_RW
    
    # Waxman-Smits parameters (fixed, make kwargs later)
    sig_water = 1/r_water                            # Water conductivity
    B_waxman  = 4.6*(1-0.6*np.exp((-sig_water)/1.3)) # Equivalent conductance per cation
    rho_0     = 2700                                 # Mineral grain density
    ...
    #   Waxman-Smits
    # TODO: Fix dependence on water saturation
    F = a*phi**(-m)                         # Formation factor
    Qv = (cec*(1-phi)*rho_0)/phi            # Charge per unit pore volume (indicative of shaliness of a formation)
    sig = (1/F)*(sig_water + (B_waxman*Qv)) # Conductivity 
    log_rt = np.log10(1/sig)                # Log vertical resistivity
...
if __name__ == '__main__':
    ...
    vp, vs, rho = gassmann(So, Sg, phi, vcl, tvd)        
